        ...
        <div class="preview-body">
          <div class="preview-left">
            ...
            <div class="plus_video_channel_des">
              ...
              />
              <!-- plus_左上角视频标题 -->
              <div class="plus_lefttop_title">{{choosedChannel}}<span v-if="secondTitle">--{{secondTitle}}</span></div>
              <!-- plus_底部视频简介 -->
              <div class="plus_des_bottom">{{productDes}}</div>
              <!-- plus_语言切换 -->
              <div class="plus_langChoose">
          ...
          <div class="preview-right">
            ...
            <a-form-model-item label="栏目选择" :labelCol="{span: 6}" :wrapperCol="{span: 18}" prop="channel">
              <a-select v-model="formPreview.channel">
                <a-select-option v-for="item in channelOptions" :key="item.id" :value="item.id" @click="choosedChannel=item.name">{{ item.name }}</a-select-option>
              </a-select>
              <template slot="extra">
                <div class="extra-tips">选择场景制作栏目需要管理员审核后才能正常显示</div>
              </template>
            </a-form-model-item>
            <!-- plus自定义标题 -->
            <div style="textAlign:left">
              <strong>二级标题: </strong>
              <input type="text" placeholder="输入自定义标题，不超过10个字" maxlength="10" v-model="secondTitle" style="marginLeft:15px;border:none;width:240px;padding:4px;outline:none">
            </div>
            <!-- plus自定义描述 -->
            <div class="plus_myDes">
              <strong style="fontSize:14px;marginRight:18px;">作品简介: </strong>
              <textarea cols="35" rows="2" style="border:none;outline:none;resize:none;padding:7px" maxlength="40" placeholder="不超过40个字" v-model="productDes"></textarea>
            </div>
            <!-- plus插入视频 -->
            <div style="textAlign:left;marginTop:15px">
              ...
              <!-- plus视频插入进度显示 -->
              <a-button type="primary" :loading="insertloading" v-if="insertloading">{{ '上传中,稍后...' }}</a-button>
            </div>
            <!-- plus视频插入框 -->
            <div class="plus_insertbox" v-if="isInsertVideo==='yes'">
              ...
              <select class="plus_insertbox_select" style="width:80px" v-model="insertPos">
...
import { add, getTaskResult, getAllChannel, sendProduct, getAudio } from '@/api/make'
...
export default {
  ...
  methods: {
    ...
    playLoadVideo () {
      ...
      const insertVideoRef = this.$refs.insertVideo
      if (this.inSertUrl) {
        if (((previewVideoRef.currentTime > previewVideoRef.duration / 2) && this.insertPos === '中间') || ((previewVideoRef.currentTime > 0.1) && this.insertPos === '开始') || ((previewVideoRef.currentTime > previewVideoRef.duration - 0.2) && this.insertPos === '结尾')) {
    ...
    saveToPreview() {
        ...
        if (valid) {
          ...
          add(formData).then( res => {
            if (res.message && res.message.code === 0) {
              ...
              // 二期修改_end
              this.taskIds.push(res.data.data.taskId)
      ...
      } else this.$message.error('配音文本缺少语种标签')
    },
    toSetting() {
      this.mode = 2
    ...
    handleAudio() {
      ...
      this.calcedText.forEach((el, index) => {
        this._getAudio(el.content, el.key, index)
      })
    },
    //  查询音频
    ...
    _sendProduct() {
      ...
      this.$refs.rulePreviewForm.validate(valid => {
        if (valid) {
          ...
          formData.append('content', this.$refs.myPreiviewQuillEditor.quill.getText().replace(reg, ''))
          ...
          formData.append('adUrl', this.insertPlayUrl) // 插入的视频
          formData.append('secondTitle', this.secondTitle + "-" + this.selectedLangs[0]) // 二级标题和主语言
          formData.append('synopsis', this.productDes) // 视频简介

          if (this.formPreview.source) {
            formData.append('source', this.formPreview.source)
          }
          this.coverFileList.forEach(el => {

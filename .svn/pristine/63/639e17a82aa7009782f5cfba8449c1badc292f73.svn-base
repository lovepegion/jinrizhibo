    ...
    <div class="product-list">
      ...
      >
        <li class="item" v-for="item in products" :key="item.id">
          <ScrutinyItem :item="item"></ScrutinyItem>
        </li>
...
import { getAllChannel, getHotSearchData, getCount, getChannelCompany } from '@/api/make'
import { getProductByPageWithAdmin } from '@/api/update.js'
import { getHotSearch } from '@/api/update.js'
import ScrutinyItem from './components/scrutinyItem.vue'
export default {
  name: 'Scrutiny',
  components: { ScrutinyItem },
  /* components: {
    Share
  }, */
  data () {
